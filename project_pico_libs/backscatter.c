...
bool generatePIOprogram(uint16_t d0,uint16_t d1, uint32_t baud, uint16_t* instructionBuffer, struct pio_program *backscatter_program, bool twoAntennas){
    ...
    uint8_t send_1_label = 6;
    uint8_t loop_1_label = send_1_label + 1;
    int16_t lastPeriodCycles1 = (((uint32_t) CLKFREQ*1000000)/baud - 5) % ((uint32_t) d1);
    int16_t lastPeriodCycles0 = (((uint32_t) CLKFREQ*1000000)/baud - 5) % ((uint32_t) d0);
    ...
    if(loop_0_label + instructionCount(d0/2, MAX_ASMDELAY) + instructionCount(d0/2 - 1, MAX_ASMDELAY) + 1 + instructionCount(tmp0, MAX_ASMDELAY) + instructionCount(max(0,lastPeriodCycles0-tmp0), MAX_ASMDELAY) + 1 >= 32){
        printf("ERROR: p1 The clock dividers are too small. The program would not fit into the state-machine instruction memory. Alternatively, you can disable the second antenna. This increaes the maximal delay per instruction from 8 to 32 cycles and thus significanlty reduces the required code space.");
        return false;
    }
	...
	instructionBuffer[3] = 0x208A ; // 0: WAIT GPIO 10 ; 0B0010_0000_1000_1010
    instructionBuffer[4] = ASM_OUT | (ASM_X_REG   << 5) |  1;       //  3: out    x, 1   
    instructionBuffer[5] = ASM_JMP_NOTX | (0x1F & send_0_label);    //  4: jmp    !x, send_0_label
    /*       symbol 1      */
    instructionBuffer[6] = ASM_MOV | (ASM_X_REG << 5) | ASM_Y_REG;  //  5: mov    x, y                  
    uint8_t length = 7;
    // full periods
...
bool generatePIOprogram2(uint16_t d0,uint16_t d1, uint32_t baud, uint16_t* instructionBuffer, struct pio_program *backscatter_program, bool twoAntennas, uint16_t phi){
    // compute label positions
    uint16_t MAX_ASMDELAY = 0x0020; // 32
    uint16_t OPT_SIDE_1   = 0x0000;
    uint16_t OPT_SIDE_0   = 0x0000;
    ...
    int16_t tmp1 = min(lastPeriodCycles1, d1/2);
    int16_t tmp0 = min(lastPeriodCycles0, d0/2);
	int16_t tmp_phi1 = floor(d1*phi/360);
	int16_t tmp_phi0 = floor(d0*phi/360);
    /*                                           pull high                 pull low            jmp                 high                                      low                            jmp  */
    uint8_t send_0_label = loop_1_label  + instructionCount(tmp_phi1, MAX_ASMDELAY) + instructionCount(d1/2, MAX_ASMDELAY) + instructionCount(d1/2 - 1 - tmp_phi1, MAX_ASMDELAY) + 1 + instructionCount(tmp1, MAX_ASMDELAY) + instructionCount(max(0,lastPeriodCycles1-tmp1), MAX_ASMDELAY) + 1;
    uint8_t loop_0_label = send_0_label + 1;

    // check that the program will fit into memory
    /*                      pull high                pull low            jmp                        high                              low                               jmp  */
    if(loop_0_label + instructionCount(tmp_phi1, MAX_ASMDELAY) + instructionCount(d0/2, MAX_ASMDELAY) + instructionCount(d0/2 - 1 - tmp_phi1, MAX_ASMDELAY) + 1 + instructionCount(tmp0, MAX_ASMDELAY) + instructionCount(max(0,lastPeriodCycles0-tmp0), MAX_ASMDELAY) + 1 >= 32){
        printf("ERROR: p2 The clock dividers are too small. The program would not fit into the state-machine instruction memory. Alternatively, you can disable the second antenna. This increaes the maximal delay per instruction from 8 to 32 cycles and thus significanlty reduces the required code space.");
        return false;
    }
    ...
    uint8_t length = 7;
    // full periods
    repeat(instructionBuffer, tmp_phi1,     ASM_SET_PINS | OPT_SIDE_0 | 0, &length, MAX_ASMDELAY);   
    repeat(instructionBuffer, d1/2,     ASM_SET_PINS | OPT_SIDE_1 | 1, &length, MAX_ASMDELAY);   //    6: set    pins, 1         side 1 [delay] 
    repeat(instructionBuffer, d1/2 - 1 - tmp_phi1, ASM_SET_PINS | OPT_SIDE_0 | 0, &length, MAX_ASMDELAY);   //  ...: set    pins, 0         side 0 [delay] 
    instructionBuffer[length] = ASM_JMP_XMM | (0x1F & loop_1_label);                             //  ...: jmp    x--, loop_1_label
    length++;
    // remaining period to fill symbol time
    repeat(instructionBuffer,                          tmp1, ASM_SET_PINS | OPT_SIDE_1 | 1, &length, MAX_ASMDELAY); //  ...: set    pins, 1         side 1 [delay] 
    ...
    length++; 
    // full periods
    repeat(instructionBuffer, tmp_phi0,     ASM_SET_PINS | OPT_SIDE_0 | 0, &length, MAX_ASMDELAY);  
    repeat(instructionBuffer, d0/2,     ASM_SET_PINS | OPT_SIDE_1 | 1, &length, MAX_ASMDELAY);    // ...: set    pins, 1         side 1 [delay_part] 
    repeat(instructionBuffer, d0/2 - tmp_phi0 - 1, ASM_SET_PINS | OPT_SIDE_0 | 0, &length, MAX_ASMDELAY);    // ...: set    pins, 0         side 0 [delay_part] 
    instructionBuffer[length] = ASM_JMP_XMM | (0x1F & loop_0_label);      //  ...: jmp    x--, loop_0_label
    length++;
    // remaining period to fill symbol time
    repeat(instructionBuffer,                          tmp0, ASM_SET_PINS | OPT_SIDE_1 | 1, &length, MAX_ASMDELAY);  //  ...: set    pins, 1         side 1 [delay_part] 
...
*/
void backscatter_program_init(PIO pio, uint sm, uint pin1, uint pin2, uint16_t d0, uint16_t d1, uint32_t baud, struct backscatter_config *config, uint16_t *instructionBuffer, bool twoAntennas, uint phase){
    ...
    if (phase)
    {
        generatePIOprogram2(d0,d1,baud, instructionBuffer, &backscatter_program, twoAntennas, phase);
    } else{
    ...
    uint32_t reps0 = ((CLKFREQ*1000000/baud - 5) / d0) - 1;
    uint32_t reps1 = ((CLKFREQ*1000000/baud - 5) / d1) - 1;
    printf("reps0 %d, reps1 %d\n", reps0, reps1);
