...
void add_header(uint8_t *packet, uint8_t seq, uint8_t *header_template)
{
    ...
    packet[HEADER_LEN - 2] = 1 + PAYLOADSIZE_ENC; // The packet length is defined as the payload data, excluding the length byte and the optional CRC. (cc2500 data sheet, p. 30)
